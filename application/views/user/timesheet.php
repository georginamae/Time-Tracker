<?php get_header('Timesheet','user timesheet'); ?>
			...
			events:[
				<?php 
					foreach($time_entry as $row){
						...
						echo '{
							title :  "'.$msg.' Task ID# '.$row->TimeEntryID.' - : '.number_format((float)$row->RenderedHours, 2, '.', '').' hrs",
							start : "'.date('Y-m-d',strtotime($row->DateInputted)).'",
							url   : "'.base_url('user/load/timesheet/'.$row->TimeEntryID.'').'"
						},';
					}
				?>
			]
